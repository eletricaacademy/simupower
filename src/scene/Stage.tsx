...
export function Stage() {
  ...
  const bg = ehEnv ? '#1b2026' : ehArc ? TEMA_SUB.bg : TEMA_BT.bg
  ...
  return (
    ...
    <Canvas
      ...
      onCreated={({ gl }) => {
        // reduz exposição na subestação branca (estava estourando)
        gl.toneMappingExposure = ehEnv ? 0.78 : 1.0
      }}
    ...
    >
      ...
      {/* iluminação */}
      <ambientLight intensity={ehArc ? 0.8 : ehEnv ? 0.4 : 0.45} />
      <hemisphereLight
        args={
          ehArc
            ? ['#f2f4f7', '#c8cacd', 0.7]
      ...
      <directionalLight
        position={[4, 7, 3]}
        intensity={ehArc ? 1.0 : ehEnv ? 0.55 : 1.2}
        castShadow={cfg.shadows && !ehEnv}
